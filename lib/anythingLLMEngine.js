...
/**
 * Perform full AnythingLLM RAG Analysis on uploaded Brief, Guidelines, Audio Notes & Materials
 */
async function analyzeBriefWithAnythingLLM(rawBriefText, fileContents = [], customApiKey = null) {
  console.log('[AnythingLLM Engine] Beginning automated RAG brief & multimedia material analysis...');
  ...
  const targetQuery = 'assignment brief requirements rubric word count citations methodology section breakdown guidelines constraints audio lecture transcript notes additional study material';
...
Analyze the provided RAG document chunks, assignment brief guidelines, audio notes transcripts, and supplementary study materials.
...
- For each section, list specific, non-negotiable points, author references, or datasets that MUST be included from the brief and attachments.

## 3. CORE THEORIES, FRAMEWORKS, AUDIO NOTES & SPECIFIC TERMINOLOGY
- Explicitly name all theories, models, formulas, frameworks, audio transcript points, or key terminology mentioned in the materials.
- Mandate where and how each concept must be applied in the text.
...
Be extremely detailed, authoritative, and specific to the uploaded materials. Output clean Markdown only.`;
  ...
  // 15-Second AbortController Timeout to allow complete RAG processing via OmniRoute
  ...
  try {
    ...
    console.log('[AnythingLLM Engine] Automated brief analysis successfully generated via OmniRoute Gateway!');
    return cleanContent;
  } catch (err) {
    clearTimeout(timeoutId);
    console.warn(`[AnythingLLM Engine Warning] OmniRoute RAG request failed: ${err.message}. Generating local fallback blueprint...`);
    return `# ANYTHINGLLM AUTOMATED BRIEF ANALYSIS & BLUEPRINT (LOCAL FAST ENGINE)

## 1. SUMMARY OF CORE REQUIREMENTS & CONSTRAINTS
- **Raw Brief & Materials Length**: ${countWords(combinedText)} words extracted across brief files and audio notes.
